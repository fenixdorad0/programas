...
namespace BunifuProProject
{
    public partial class Form1 : Form
    {
        ...
        private void calcularTrapecio_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Verificar A y B ");
            }
        }
        ...
        private void calcularBisecion_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("verifique xl y xu");
            }
        }
        ...
        {
            /*
            double ximenos1,xi, fxi, fxmenosl, ximas1, error = 100;
            listBox1.Items.Clear();
            try
            {
                
                ximenos1 = Convert.ToDouble(textBox1.Text);
                xi = Convert.ToDouble(textBox2.Text);
                double limError = Convert.ToDouble(textBox3.Text);
                double limItera = Convert.ToDouble(textBox4.Text);
                int cont1 = 0;
                //xr = (xl + xu) / 2;
                fxmenosl = fBisecion(ximenos1);
                fxi = fBisecion(xi);
                ximas1 = (xi - (((fxi) * (ximenos1 - xi)) / (fxmenosl - fxi)));
                ...
                while (Math.Abs(error) > limError && cont1 < limItera)
                {
                    ...
                    else
                    {
                        ...
                        ximas1 = (ximenos1 + xi) / 2;
                        fxmenosl = fBisecion(ximenos1);
                        fxi = fBisecion(xi);
                        fxr = fBisecion(ximas1);
                        fxlporfxr = fxmenosl * fxr;
                        error = ((ximas1 - xra) / ximas1) * 100;
                        xra = ximas1;
                    }
                    String texto =

                                    "i= " + cont1 + "   " +
                                    "XL= " + Convert.ToString(Decimal.Round(Convert.ToDecimal(ximenos1), 3)) + "   " +
                                    "Xu= " + Convert.ToString(Decimal.Round(Convert.ToDecimal(xi), 3)) + "   " +
                                    "XR= " + Convert.ToString(Decimal.Round(Convert.ToDecimal(ximas1), 4)) + "   " +
                                    "error= " + Convert.ToString(Decimal.Round(Convert.ToDecimal(Math.Abs(error)), 3))
                                    ;
                    listBox1.Items.Add(texto);
                    cont1 = cont1 + 1;
        ...
        public static double fsecante(double x)
        {
